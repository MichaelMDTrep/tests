...
// import { injected, portis, fortmatic } from '../../../utils/connectors';
...
const SignInModal = ({ initContracts }: IHeader): JSX.Element => {
    ...
    const forgotPassword = async (e: any) => {
        ...
        try {
            ...
            setNotificationMessage(
                <Alert variant="filled" severity="success">
                    If your email matches your username, you will receive a new reset password
                </Alert>,
            );
        } catch (error) {
            setOpenError(true);
    ...
    const handleUserSignUp = async (e: any) => {
        ...
        axios
            ...
            .catch(() => {
                ...
                setNotificationMessage(
                    <Alert variant="filled" severity="error">
                        There was an error signing you up! Please make sure that your account email is unique
                    </Alert>,
                );
                setLoading(false);
            });
    ...
    const wallet = (
        <>
            ...
            >
                <DialogContent>
                    <ConnectWalletModal hidden={!state.signInModalIsOpen} initContracts={initContracts} />
                </DialogContent>
            </Dialog>
        </>
    );
                            ...
                            <DialogActions>
                                <Grid container direction="column" justify="center" alignItems="center" spacing={2}>
                                    <Grid item xs={12}>
                                        <>
                                            {loading && <CircularProgress size={24} className="progress" />}
                                            <Button type="submit" disabled={loading} className="LoginButton-header">
                                                Sign in
                                            </Button>
                                        </>
                                    </Grid>
                                    ...
                                    <Grid item xs={12}>
                                        <Typography>
                                            Forgot Password?
                                            <Link onClick={() => setForgotPass(true)}> Click Here</Link>
                                        </Typography>
                                    </Grid>
                                    <Grid item xs={12}>
                                        <Typography>
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        <Grid container direction="column" justify="center" alignItems="center" spacing={2}>
                            ...
                            >
                                Click on the link in that email to finish creating your account. If you don't see it,
                                you may have to check your <strong>spam folder</strong>.
                            </p>
                            <p
                                style={{
                                    display: 'inline-block',
    ...
    if (newSignUp) {
        ...
        return !signIn ? signInModal : signUpModal;
    } else {
        // initContracts();
        return wallet;
    }
