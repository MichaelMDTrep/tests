...
export function TokenProvider({ children }: { children: ReactNode }): JSX.Element {
    const { chainId } = useWeb3React<Web3Provider>();

    const CURRENT_ADDRESS = chainId === 1 ?  MAINNET_ADDRESS : (chainId === 3? ROPSTEN_ADDRESS : RINKEBY_ADDRESS);

    const tokenInstance = useContract(CURRENT_ADDRESS, SuperWorldToken.abi, true);

    return <TokenContext.Provider value={{ tokenInstance }}>{children}</TokenContext.Provider>;
}
